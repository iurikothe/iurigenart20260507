...
| Token | `1000286` |
| Project ID | `1` |
| Contract | `0x145789247973c5d612bf121e9e4eef84b63eb707` |
| Contract flavor | Engine — Art Blocks × Bright Moments |
| Chain | ethereum |
| Script type | `p5@1.0.0` |
| Aspect ratio | 1.0 |
| Invocations | 924 |
| Project status | locked=False, complete=True, paused=True |
| Completed at | 2023-08-19T17:01:35+00:00 |
| Artist site | https://www.brightmoments.io/923emptyrooms |

- **Generator**: <https://generator.artblocks.io/1/0x145789247973c5d612bf121e9e4eef84b63eb707/1000286>
- **Art Blocks page**: <https://www.artblocks.io/collection/0x145789247973c5d612bf121e9e4eef84b63eb707-1>
...
- p5.js + raw WebGL (custom GLSL shaders inlined as template strings). Uses `p5.disableFriendlyErrors = true` for perf.
- Shader pipeline draws 3D 'rooms' as line geometry whose vertex Z-positions are sampled from a displacement-map texture — `getLineFromDisplacementMap()` in the vertex shader does the lookup. The per-piece 'still life' texture is what makes each mint a distinct room.
- RGB channels treated separately (`displayRed/Green/Blue`) — different combinations make different rooms (Berlin / NY / CDMX / London / Tokyo / LA, hinted by `cla, cny, cberlin, ccdmx, clondon, ctokyo`).
- Animation parameters (`speeds = [0.25, 0.5, 1.0, 2.0, 0.0]`) include zero — a frozen still-life mode is a valid trait outcome.
- REAS uses `var seed` here vs `let seed` in Pre-Process — small style detail, two years apart.
